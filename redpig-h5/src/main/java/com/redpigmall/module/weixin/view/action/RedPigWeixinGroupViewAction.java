...
@Controller
public class RedPigWeixinGroupViewAction extends BaseAction{
	...
	public ModelAndView group(HttpServletRequest request,
		...
		ModelAndView mv = new RedPigJModelAndView("weixin/group.html",
		...
		if (("".equals(type)) || ("goods".equals(type)) || (type == null)) {
			...
			if (groups.size() > 0) {
				...
				if ((ga_id != null) && (!ga_id.equals(""))) {
					...
					if ((ids != null) && (ids.size() > 0)) {
						if ((cga_id != null) && (!cga_id.equals(""))) {
							maps.put("gg_gc_ids", genericAreaIds(CommUtil.null2Long(cga_id)));
						} else {
				...
				GroupPriceRange gpr = this.groupPriceRangeService
						.selectByPrimaryKey(CommUtil.null2Long(gpr_id));
				if (gpr != null) {
		...
		if ("life".equals(type)) {
			mv = new RedPigJModelAndView("weixin/group_life.html",
					this.configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request,
					response);
			if ((ga_id != null) && (!ga_id.equals(""))) {
				request.getSession(false).setAttribute("ga_id", ga_id);
			} else {
				ga_id = (String) request.getSession(false)
						.getAttribute("ga_id");
			}
			Map<String, Object> params = Maps.newHashMap();
			...
			if (groups.size() > 0) {
				...
				Area group_area = this.areaService.selectByPrimaryKey(CommUtil.null2Long(ga_id));
				
				if ((ga_id != null) && (!ga_id.equals(""))) {
					switch (group_area.getLevel()) {
					case 1:
						maps.put("gg_gai_area_parent_id", group_area.getId());
						break;
					case 2:
						maps.put("gg_gai_area_id", group_area.getId());
						break;
					}
					mv.addObject("ga", group_area);
				}
				...
				params.put("weixin_shop_recommend", 1);
				params.put("group_status", 1);
				params.put("weixin_shop_recommend", 1);
				
				List<GroupLifeGoods> recommend_group_goods = this.groupLifeGoodsService.queryPageList(params);
				
				if (recommend_group_goods.size() > 0) {
					mv.addObject("recommend", recommend_group_goods);
				}
				mv.addObject("group", groups.get(0));
				mv.addObject("gc_id", gc_id);
				mv.addObject("gpr_id", gpr_id);
		...
		if (("life".equals(type)) && (ga_id != null) && (!ga_id.equals(""))) {
			mv.addObject("ga",
					this.areaService.selectByPrimaryKey(CommUtil.null2Long(ga_id)));
		}
		...
		if (CommUtil.null2String(orderType).equals("desc")) {
			...
			mv.addObject("c_orderType", "d");
		}
		return mv;
	}

	@RequestMapping({ "/group/data" })
	...
	@RequestMapping({ "/grouplife/grouplifegoods_list" })
	...
	@RequestMapping({ "/grouplife/groupAreaChose" })
	...
	@RequestMapping({ "/grouplife/grouplifegoods_search" })
	...
	@RequestMapping({ "/grouplife/areaSearch" })
