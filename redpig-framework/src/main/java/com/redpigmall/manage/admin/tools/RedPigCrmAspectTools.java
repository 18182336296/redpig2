...
public class RedPigCrmAspectTools {
	...
	private static final String ORDER_AOP = "execution(* com.redpigmall.service.RedPigOrderFormService.*(..))";
	private static final String CONSULT_AOP = "execution(* com.redpigmall.service.RedPigConsultService.*(..))";
	private static final String FAVORITE_AOP = "execution(* com.redpigmall.service.RedPigFavoriteService.*(..))";
	...
	public void order_aop(JoinPoint point) {
		...
		if ((point.getArgs().length > 0)
				&& (point.getSignature().getName().equals("saveEntity"))) {
