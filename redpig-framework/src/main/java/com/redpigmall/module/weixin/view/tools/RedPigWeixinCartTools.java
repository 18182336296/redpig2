...
public class RedPigWeixinCartTools {
	...
	public List<GoodsCart> getothergc(String id) {
	...
	public boolean coupon_count(String id) {
		List<Coupon> coupons = this.couponViewTools.getUsableCoupon(CommUtil.null2Long(id), SecurityUserHolder.getCurrentUser().getId());
		boolean ret = false;
		if (coupons.size() > 0) {
			ret = true;
		}
