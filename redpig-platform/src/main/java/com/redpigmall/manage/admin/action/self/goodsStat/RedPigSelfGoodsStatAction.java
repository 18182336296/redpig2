package com.redpigmall.manage.admin.action.self.goodsStat;
...
/**
 * <p>
 * Title: RedPigSelfGoodsStatAction.java
 * </p>
 * 
 * <p>
...
@Controller
public class RedPigSelfGoodsStatAction extends BaseAction{
	/**
	 * 商品统计
	 * @param request
	 * @param response
	 * @return
	 */
	@SecurityMapping(title = "商品统计", value = "/self_stat_items*", rtype = "admin", rname = "商品统计", rcode = "self_stat_goods", rgroup = "自营")
	@RequestMapping({ "/self_stat_items" })
	public ModelAndView stat_goods(HttpServletRequest request,
			HttpServletResponse response) {
		ModelAndView mv = new RedPigJModelAndView("admin/blue/stat_self_goods.html",
	...
	@SecurityMapping(title = "有信息的商品", value = "/self_stat_goods_list*", rtype = "admin", rname = "商品统计", rcode = "self_stat_goods", rgroup = "自营")
	@RequestMapping({ "/self_stat_goods_list" })
	 ...
	 */
	@SuppressWarnings({ "rawtypes"})
	@SecurityMapping(title = "商品统计结果", value = "/self_stat_goods_done*", rtype = "admin", rname = "商品统计", rcode = "self_stat_goods", rgroup = "自营")
	@RequestMapping({ "/self_stat_goods_done" })
		...
		if ((ids != null) && (!ids.equals(""))) {
			...
			if (arr.length == 1) {
					...
					while (!cal.getTime().after(end)) {
						...
						for (GoodsLog gl : log) {
							...
							Map<String, Integer> specmap = (Map) JSON
									.parseObject(jsonstr, new TypeReference() {
									}, new Feature[0]);
				...
				if ((begin != null) && (end != null)) {
					...
					Map<String, Object> params = Maps.newHashMap();
					List<GoodsLog> log = null;
					label3323: while (!cal.getTime().after(end)) {
						...
						for (String id : arr) {
							...
							for (GoodsLog gl : log) {
								...
								if (!goods.containsKey(id)) {
									goods.put(Long.valueOf(tempid),gl.getGoods_name());
								}
								if (clickcount
	 ...
	 */
	@SuppressWarnings("rawtypes")
	@SecurityMapping(title = "全商城统计结果", value = "/self_stat_all*", rtype = "admin", rname = "商品统计", rcode = "self_stat_goods", rgroup = "自营")
	@RequestMapping({ "/self_stat_all" })
	public ModelAndView stat_all(HttpServletRequest request,
		...
		if ((begin != null) && (end != null)) {
			...
			mv.addObject("end", end);
			mv.addObject("times", JSON.toJSONString(times));
			mv.addObject("timeslength", Integer.valueOf(CommUtil.null2Int(Integer.valueOf(times.size() / 9))));
			mv.addObject("goods_click", JSON.toJSONString(goods_click));
