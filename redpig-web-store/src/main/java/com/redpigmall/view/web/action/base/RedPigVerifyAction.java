...
public class RedPigVerifyAction extends BaseAction{
	...
	public void verity_password(HttpServletRequest request,
		...
		/*
		 * .equalsIgnoreCase()函数：
         * 比较的参数只能是字符串，这里只要字符串的长度相等，
         * 字母的大小写是忽略的。认为A-Z和a-z是一样的。
		 */
		if("admin".equalsIgnoreCase(user_list.get(0).getUserRole())){
			ret = "adminerror";
		}
...
//		}
			...
			String url = CommUtil.getURL(request) + "/seller_index";
			try {
	...
	//@RequestMapping({ "/skip_index" })
	@ResponseBody
	public String skip_index(HttpServletRequest request,
			HttpServletResponse response, String password, String username,
			String cod) {
