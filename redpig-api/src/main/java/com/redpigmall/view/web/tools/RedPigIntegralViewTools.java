	...
	public Map query_user_level(String id) {
		...
		if (this.configService.getSysConfig().getUser_level() != null) {
			if (!this.configService.getSysConfig().getUser_level().equals("")) {
				...
				int goods_fee = Math.round(CommUtil.null2Float(user.getIntegral()));
				...
				if (maps.size() >= 1) {
					...
					if (heigh == null) {
						for (Map level_map : maps) {
							if (heigh == null) {
								heigh = level_map;
							} else if (CommUtil.null2Int(heigh.get("level")) < CommUtil.null2Int(level_map.get("level"))) {
								heigh = level_map;
							}
						}
					}
					...
					if (CommUtil.null2Int(heigh.get("credit_down")) <= goods_fee) {
						...
						for (Map map : maps) {
							if ((CommUtil.null2Int(map.get("credit_up")) != -1)
									&& (CommUtil.null2Int(map.get("credit_down")) <= goods_fee)
