...
public class RedPigEvaluateViewTools {
	...
	@Autowired
	private RedPigFreeApplyLogService freeapplylogService;
...
//		if (eva.equals("200")) {
//			params.put("goods_id", "");
//			List localList = this.freeapplylogService.queryPageList(params);
//		}
		params.put("evaluate_type", "goods");
		params.put("evaluate_status", Integer.valueOf(0));
		if (eva.equals("100")) {
			params.put("evaluate_photos_no", "");
			...
			params.put("evaluate_buyer_val",Integer.valueOf(CommUtil.null2Int(eva)));
			params.put("evaluate_goods_id", CommUtil.null2Long(goods_id));
	...
	public List<FreeApplyLog> queryByfal(String goods_id) {
		...
		List<FreeApplyLog> list = this.freeapplylogService.queryPageList(params);
		...
		if ((ids != null) && (!ids.equals(""))) {
			for (String str : ids.split(",")) {
				if ((str != null) && (!str.equals(""))) {
					Accessory img = this.accessoryService.selectByPrimaryKey(CommUtil.null2Long(str));
					if (img != null) {
						list.add(img);
					}
				}
