...
/**
 * <p>
 * Title: RedPigGoodsViewTools.java
 * </p>
 * 
 * <p>
 * Description: 商品相关处理工具类
 * </p>
 * 
 * <p>
	...
	public List<GoodsSpecification> generic_spec(String id) {
		...
		if ((id != null) && (!id.equals(""))) {
			...
			for (GoodsSpecProperty gsp : goods.getGoods_specs()) {
				...
				gsp = this.goodsSpecPropertyService.selectByPrimaryKey(gsp.getId());
		...
		for (int i = 0; i < specs.size() - 1; i++) {
			for (int j = specs.size() - 1; j > i; j--) {
				if (specs.get(j).getId().equals(specs.get(i).getId())) {
					specs.remove(j);
				}
			}
		}  
