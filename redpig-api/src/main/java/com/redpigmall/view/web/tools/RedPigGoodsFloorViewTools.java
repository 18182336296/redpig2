...
public class RedPigGoodsFloorViewTools {
		...
		if ((json != null) && (!json.equals(""))) {
			try {
				...
				for (int i = 1; i <= 10; i++) {
					...
					params.put("ids", Lists.newArrayList(CommUtil.null2Long(map.get(key))));
	...
	public Map generic_goods_jd_list(String json) {
		Map<String, Object> map = Maps.newHashMap();
		map.put("list_title", "商品排行");
		if ((json != null) && (!json.equals(""))) {
			try {
	...
	public Map generic_style2_goods(String json, String module_id) {
		try {
			...
			for (Map map : maps) {
				if (map.get("module_id").equals(module_id)) {
					String href_url = (String) map.get("href_url");
					String img_url = (String) map.get("img_url");
					map.put("href_url",
							href_url.substring(href_url.lastIndexOf("/")));
		...
		try {
			...
			for (Map map : maps) {
				if (map.get("module_id").equals(module_id)) {
					...
					map.put("href_url",href_url.substring(href_url.lastIndexOf("/")));
