 ...
 * 
 * <p>
 * Title: RedPigDatabaseTools.java
 * </p>
 * 
 * <p>
 ...
 */
@Repository
public class RedPigDatabaseTools implements IBackup {

	@Autowired
	private PublicMethod publicMethod;

	public RedPigDatabaseTools() {
	}
	...
	public boolean createSqlScript(HttpServletRequest request, String path,
		...
		int count = 1;
		boolean ret = true;
		float psize = RedPigCommUtil.null2Float(size);
		...
		try {
			...
			for (String table : backup_list) {
				...
				for (String line : list) {
					// 及时计算分卷文件大小
					double fsize = RedPigCommUtil.div(file.length(), 1024);
					if (fsize > psize) {
						pwrite.flush();
						// pwrite.close();
						count++;
		...
		try {
			...
			for (String table : backup_list) {
				if (!RedPigCommUtil.null2String(table).equals("")) {
					// 生成insert语句
