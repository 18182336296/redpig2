...
public class IPSeeker {
	...
	/** 保存的文件夹 */
	private String INSTALL_DIR = ClusterSyncTools.getCDN() + "resources" + File.separator + "data";
	/** 一些固定常量，比如记录长度等等 */
	...
	public String getCountry(byte[] ip) {
		...
		// 检查ip地址文件是否正常
		if (this.ipFile == null) {
			return this.Message.bad_ip_file;
		}
		// 保存ip，转换ip字节数组为字符串形式
	...
	private IPLocation getIPLocation(byte[] ip) {
		...
		if (info == null) {
			info = new IPLocation();
			info.setCountry(this.Message.unknown_country);
			info.setArea(this.Message.unknown_area);
		}
		return info;
	}
	...
	private String readArea(long offset) throws IOException {
		...
		if ((b == 1) || (b == 2)) {
			...
			if (areaOffset == 0L) {
				return this.Message.unknown_area;
			}
